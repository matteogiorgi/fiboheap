...
static uint64_t touch_csr_graph(const CsrGraph *g) {
    uint64_t acc = 0;
...
static int graphvec_push(GraphVec *v, CsrGraph g) {
    if (v->size == v->capacity) {
        size_t new_cap = (v->capacity == 0) ? 4 : v->capacity * 2;
        CsrGraph *tmp = realloc(v->graphs, new_cap * sizeof(CsrGraph));
        if (!tmp) {
            return 1;
        }
        v->graphs = tmp;
...
int main(int argc, char **argv) {
    ...
    for (size_t iter = 0; max_copies == 0 || iter < max_copies; ++iter) {
        ...
        uint64_t chk = touch_csr_graph(&g);
        global_checksum ^= chk;
        ...
        double now = now_sec();
        printf("CSR copy %zu loaded: n=%u m=%u checksum=%" PRIu64 " elapsed=%.3f s\n",
               vec.size, g.n, g.m, chk, now - t0);
        fflush(stdout);
    }

    printf("\nCSR total copies kept in memory: %zu\n", vec.size);
    printf("CSR global checksum: %" PRIu64 "\n", global_checksum);
